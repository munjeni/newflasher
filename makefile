...
CROSS_CFLAGS=${CFLAGS} -I include -I zlib-1.2.13 -L zlib-1.2.13 -I expat-2.2.9/lib -L expat-2.2.9/lib/.libs
...
libs:
	@mkdir -p include
	@test -d zlib-1.2.13 && echo "" || wget https://zlib.net/zlib-1.2.13.tar.gz
	@test -d zlib-1.2.13 && echo "" || tar xzf zlib-1.2.13.tar.gz
...
newflasher.x64: libs newflasher.c version.h
	@cd zlib-1.2.13 && CC=gcc ./configure --static && make clean && make
...
newflasher.i386: libs newflasher.c version.h
	@cd zlib-1.2.13 && CC="gcc -m32" ./configure --static && make clean && make
...
newflasher.arm32: libs newflasher.c version.h
	@cd zlib-1.2.13 && CC=${ARMCC} ./configure --static && make clean && make
...
newflasher.arm64: libs newflasher.c version.h
	@cd zlib-1.2.13 && CC=${ARMCC64} ./configure --static && make clean && make
